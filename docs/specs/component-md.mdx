...
<Steps>
  <Step title="Preflight + prepare">
    The orchestrator shells out to `uspec-skills component-md prepare`, which validates `_base.json` against the bundled plugin schema, stages it into `.uspec-cache/{componentSlug}/`, and writes a prepare manifest plus four compact evidence slices. The agent parses the JSON manifest and uses those slices to steer the downstream specialists.
  </Step>
